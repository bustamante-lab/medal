...
pat1 <- getClusterScores(profiles, 1, outcomes, maxDay) %>% mutate(cluster=1)
pat2 <- getClusterScores(profiles, 2, outcomes, maxDay) %>% mutate(cluster=2)
pat3 <- getClusterScores(profiles, 3, outcomes, maxDay) %>% mutate(cluster=3)
pat4 <- getClusterScores(profiles, 4, outcomes, maxDay) %>% mutate(cluster=4)
pat5 <- getClusterScores(profiles, 5, outcomes, maxDay) %>% mutate(cluster=5)
pat6 <- getClusterScores(profiles, 6, outcomes, maxDay) %>% mutate(cluster=6)
...
fit1 <- lm(formula=gi_new+cluster~daysSinceOnset, data=pat)
