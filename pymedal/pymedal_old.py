import numpy as np
import pdb
from numba import  jit, autojit, njit, prange
import numba as nb
import pandas as pd
import time
import tqdm
...
events = pd.read_csv('../medication/medsEvents.csv')
meds   = pd.read_csv('../medication/medsDictonary.csv')
...
np.savetxt("patientID.txt", np.unique(data.id).astype(int), fmt='%i')
...
@njit(nb.int32[:,:](nb.int8[:], nb.int8[:], nb.int32[:,:]), nopython=True, nogil=True,  parallel=True, cache=True)
...
def medalDistance(seq1, seq2):
    ...
    l1 = len(seq1) + 1
    mat = np.empty((l1, l1), dtype=np.int32)
...
#pr = cProfile.Profile()
#pr.enable()
for i in tqdm.tqdm(range(npatients)):
...
#np.savetxt("patientID.txt", np.unique(data.id).astype(int), fmt='patient %i')
